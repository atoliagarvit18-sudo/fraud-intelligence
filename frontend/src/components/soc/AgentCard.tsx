...
const meta: Record<AgentKey, { title: string; icon: typeof AudioLines }> = {
  speech: { title: "Speech Intelligence", icon: AudioLines },
  text: { title: "Text Intelligence", icon: FileText },
  visual: { title: "Visual Intelligence", icon: ScanEye },
...
export function AgentCard({ agentKey, data, linkTo = true }: { agentKey: AgentKey; data: AgentResult; linkTo?: boolean }) {
  const { title, icon: Icon } = meta[agentKey];
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[11px] uppercase tracking-widest text-muted-foreground">Agent</div>
          </div>
        </div>
